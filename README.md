...
**Unload** 是一套聚焦「職場心理邊界」的覺察工具，同時也是一個以 **MIT 授權** 釋出的開源專案。目前仍處於長期 **Beta 測試階段**，功能與介面會隨著回饋持續調整。

透過引導式的紀錄流程與視覺化整理介面，Unload 協助你把工作中的壓力、責任感與邊界混亂，拆解成比較能被看見與承接的片段，為高敏族群與在意邊界的人留出一點喘息空間。
...
Unload 會根據實作環境，搭配後端服務與瀏覽器端儲存機制來保存資料，例如：
